...
beforeEach(() => {
  guestsResult.value = {
    data: [
      { id: "1", created_at: "2026-04-01", name: "M", email: "m@x", phone: "9", companion_count: 1, companion_names: ["A"], token: "t1", checked_in_day1_at: new Date().toISOString(), checked_in_day2_at: null, email_sent_at: null },
      { id: "2", created_at: "2026-04-02", name: "B", email: "b@x", phone: "9", companion_count: 0, companion_names: [], token: "t2", checked_in_day1_at: null, checked_in_day2_at: null, email_sent_at: null },
    ],
    error: null,
  };
});
...
describe("AdminPage dashboard", () => {
  it("calcula stats: inscritos, acompanhantes, check-in dia 1/2, pendentes", async () => {
    ...
    expect(container.textContent).toContain("Check-in Dia 1");
    expect(container.textContent).toContain("Check-in Dia 2");
  ...
  it("companion_count null tratado como 0", async () => {
    guestsResult.value = {
      data: [{ id: "1", created_at: "x", name: "x", email: "x", phone: "x", companion_count: null, companion_names: [], token: "t", checked_in_day1_at: null, checked_in_day2_at: null, email_sent_at: null }],
      error: null,
    };
    const { default: Page } = await import("@/app/admin/(authed)/page");
    const ui = await Page();
